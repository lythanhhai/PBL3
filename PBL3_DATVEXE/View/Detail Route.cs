...
namespace PBL3_DATVEXE
{
    public partial class Detail_Route : Form
    {
        public Detail_Route()
        {
            ...
            setCBBLSH();
            Show1(((CBBitem)bunifuDropdown2.SelectedItem).Value,"");
            //  SetCBBSort();
        }
       ...
       /* private void bunifuButton2_Click(object sender, EventArgs e)
        {
            ...
            {
                bunifuDataGridView1.DataSource = BLL_delRoute.Instance.sort(new BLL_delRoute.Compare(DTO_DelRoute.compareid), ((CBBitem)bunifuDropdown2.SelectedItem).Value, bunifuTextBox1.Text);
            }
            if (bunifuDropdown1.SelectedItem.ToString() == "route")
            {
                bunifuDataGridView1.DataSource = BLL_delRoute.Instance.sort(new BLL_delRoute.Compare(DTO_DelRoute.compareroute), ((CBBitem)bunifuDropdown2.SelectedItem).Value, bunifuTextBox1.Text);
            }
            if (bunifuDropdown1.SelectedItem.ToString() == "vehicle")
            {
                bunifuDataGridView1.DataSource = BLL_delRoute.Instance.sort(new BLL_delRoute.Compare(DTO_DelRoute.comparevehicle), ((CBBitem)bunifuDropdown2.SelectedItem).Value, bunifuTextBox1.Text);
            }
            if (bunifuDropdown1.SelectedItem.ToString() == "date")
            {
                bunifuDataGridView1.DataSource = BLL_delRoute.Instance.sort(new BLL_delRoute.Compare(DTO_DelRoute.Comparedate), ((CBBitem)bunifuDropdown2.SelectedItem).Value, bunifuTextBox1.Text);
            }
            if (bunifuDropdown1.SelectedItem.ToString() == "time_start")
            {
                bunifuDataGridView1.DataSource = BLL_delRoute.Instance.sort(new BLL_delRoute.Compare(DTO_DelRoute.comparetime), ((CBBitem)bunifuDropdown2.SelectedItem).Value, bunifuTextBox1.Text);
            }
            if (bunifuDropdown1.SelectedItem.ToString() == "price")
            {
                bunifuDataGridView1.DataSource = BLL_delRoute.Instance.sort(new BLL_delRoute.Compare(DTO_DelRoute.comparenumber), ((CBBitem)bunifuDropdown2.SelectedItem).Value, bunifuTextBox1.Text);
            }
            if (bunifuDropdown1.SelectedItem.ToString() == "deleted")
            {
                bunifuDataGridView1.DataSource = BLL_delRoute.Instance.sort(new BLL_delRoute.Compare(DTO_DelRoute.Comparedele), ((CBBitem)bunifuDropdown2.SelectedItem).Value, bunifuTextBox1.Text);
            }
        }*/
    }
}
